...
const dot_files_dir = path.join(data_dir, 'test/dot_files');
const expected_dir = path.join(dot_files_dir, 'expected');
const actual_dir = path.join(dot_files_dir, 'actual');
...
for (const json_file of json_files) {
    ...
    if (overwrite_expected) {
        const expected_path = path.join(expected_dir, dot_name);
        ...
        if (!fs.existsSync(expected_path)) {
            ...
            const actual_path = path.join(actual_dir, dot_name);
