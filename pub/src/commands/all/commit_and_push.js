...
const { clean_project } = require('../../lib/clean_utils');
const { analyze_dependencies, get_build_order } = require('../../lib/dependency_graph_utils');
const determineCommitReadinessModule = require('../../lib/determine_commit_readiness');
...
// Load structure.json for validation
...
// Analyze dependencies to determine proper order
...
// Get commit order (topologically sorted - dependencies first)
let commit_order;
try {
...
for (const project of commit_order) {
    ...
    try {
        ...
        if (!is_force) {
            ...
            const clean_result = clean_project(project.path, { verbose: false, use_git: true });
            if (!clean_result.success) {
                console.error(`  ❌ Clean failed for ${get_relative_path(project.path)}`);
                clean_result.errors.forEach(err => console.error(`     ${err}`));
                failed_repos.push({ name: project.name, path: project.path, reason: 'Clean failed' });
                failed_count++;
                continue;
            }
