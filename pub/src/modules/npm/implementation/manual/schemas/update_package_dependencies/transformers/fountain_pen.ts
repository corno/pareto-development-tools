...
export const Error: Error = ($) => {
    return _p.decide.state($, ($) => {
        switch ($[0]) {
            case 'could not remove node_modules': return _p.ss($, ($) => sh.ph.composed([
                sh.ph.literal("could not remove node_modules: "),
                t_remove_to_fountain_pen.Error($)
            ]))
            case 'could not remove package-lock.json': return _p.ss($, ($) => sh.ph.composed([
                sh.ph.literal("could not remove package-lock.json: "),
                t_remove_to_fountain_pen.Error($)
            ]))
            case 'could not update to latest': return _p.ss($, ($) => sh.ph.composed([
                sh.ph.literal("could not update to latest typescript dependencies: "),
                t_u2l_to_fountain_pen.Error($)
            ]))
            case 'could not install dependencies': return _p.ss($, ($) => sh.ph.composed([
