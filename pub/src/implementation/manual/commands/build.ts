...
export const $$: signatures.commands.build = _p.command_procedure(
    ($p, $cr) => [
        $cr.remove.execute(
            ...
            ($): d.Error => ['error removing pub dist dir', { 'path': s_path.Context_Path($p.path), 'error': $ }],
        ),
        $cr.remove.execute(
            {
            ...
            ($): d.Error => ['error removing test dist dir', { 'path': s_path.Context_Path($p.path), 'error': $ }],
        ),
        $cr.tsc.execute(
            {
                'path': _p.optional.set(t_path_to_path.create_node_path($p.path, `pub`)),
            },
            ($): d.Error => ['error building pub', {
                'path': s_path.Context_Path($p.path),
                'error': $,
            }],
        ),
        $cr.tsc.execute(
            {
                'path': _p.optional.set(t_path_to_path.create_node_path($p.path, `test`)),
            },
