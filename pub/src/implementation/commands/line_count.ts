...
import * as t_line_count_to_line_count from "../transformers/directory_content/line_count"
    ...
    ($p, $cr, $q) => [
        ...
        _easync.p.query_without_error_transformation(
            ...
            ($v) => [
                $cr.log.execute(
                    {
                        'lines': op_flatten_list<string>($v.deprecated_to_array(() => 1).map(($): _et.List<string> => {
                            const package_name = $.key
                            return t_line_count_to_line_count.dict_to_list(t_line_count_to_line_count.Directory2(t_line_count_to_line_count.Directory($.value))).map(($) => `${package_name},${$['path']},${$['line count']}`)
                        }))
                    },
                    ($): d.Error => ['log', $],
                )
