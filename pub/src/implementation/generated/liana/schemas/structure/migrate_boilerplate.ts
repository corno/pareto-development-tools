...
export const Directory: t_signatures.Directory = ($) => _p.deprecated_cc($, ($): t_out.Directory => {
    switch ($[0]) {
        case 'dictionary': return _p.ss($, ($) => ['dictionary', Directory(
            $
        )])
        case 'group': return _p.ss($, ($) => ['group', $.__d_map(($) => _p.deprecated_cc($, ($): t_out.Directory.group.D => {
            switch ($[0]) {
                case 'directory': return _p.ss($, ($) => ['directory', Directory(
                    $
                )])
                case 'file': return _p.ss($, ($) => ['file', _p.deprecated_cc($, ($): t_out.Directory.group.D.file => {
                    switch ($[0]) {
                        case 'manual': return _p.ss($, ($) => ['manual', null])
        ...
        case 'ignore': return _p.ss($, ($) => ['ignore', null])
        case 'generated': return _p.ss($, ($) => ['generated', ({
            'commit to git': _p.deprecated_cc($['commit to git'], ($) => $),
        })])
        default: return _p.au($[0])
