...
exports.project_cluster_state_to_dot = project_cluster_state_to_dot;
/**
 ...
 */
function project_cluster_state_to_dot(cluster_state, options = {}) {
    ...
    const projects = [];
    const all_project_names = new Set();
    ...
    for (const [project_name, project_entry] of Object.entries(cluster_state.projects)) {
        ...
        const has_issues = (project_state.structure[0] === 'invalid' ||
            project_state.test[0] === 'failure' ||
            project_state.git['staged files'] ||
            project_state.git['dirty working tree'] ||
            Object.values(project_state.dependencies).some(dep => dep.target[0] === 'not found' ||
