...
exports.differs_from_published = differs_from_published;
 ...
 */
function differs_from_published(package_path) {
    ...
    try {
        ...
        try {
            const npm_view_output = (0, child_process_1.execSync)(`npm view ${package_name} version`, {
                stdio: 'pipe',
                encoding: 'utf8'
            }).trim();
 ...
 */
function create_comparison_dirs(package_name) {
 ...
 */
function extract_published_package(package_name, published_dir) {
    // Download and extract published package
    (0, child_process_1.execSync)(`npm pack ${package_name}`, {
        cwd: published_dir,
        stdio: 'pipe'
    });
    const tgz_files = fs.readdirSync(published_dir).filter(file => file.endsWith('.tgz'));
    if (tgz_files.length > 0) {
        (0, child_process_1.execSync)(`tar -xzf "${tgz_files[0]}"`, {
            cwd: published_dir,
            stdio: 'pipe'
        });
 ...
 */
function extract_local_package(pub_path, local_dir) {
    // Create local package
    (0, child_process_1.execSync)(`npm pack "${pub_path}"`, {
        cwd: local_dir,
        stdio: 'pipe'
    });
    const local_tgz_files = fs.readdirSync(local_dir).filter(file => file.endsWith('.tgz'));
    if (local_tgz_files.length > 0) {
        (0, child_process_1.execSync)(`tar -xzf "${local_tgz_files[0]}"`, {
            cwd: local_dir,
            stdio: 'pipe'
        });
 ...
 */
function move_package_contents(source_dir, target_dir) {
 ...
 */
function compare_package_contents(published_dir, local_dir) {
    try {
