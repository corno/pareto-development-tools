...
if ((dist_only + node_modules_only + git_clean) > 1) {
...
// Handle git cleaning
if (git_clean) {
    console.log('Cleaning files listed in .gitignore...');
    const result = clean_project(package_path, { verbose, use_git: true });
    // Report results
    if (result.success) {
        if (result.cleaned.length > 0) {
            console.log(`✓ Successfully cleaned: ${result.cleaned.join(', ')}`);
        }
        else {
        ...
        console.error(`❌ Git cleaning failed for ${result.project_name}`);
...
const result = clean_project(package_path, {
    verbose,
    dist_only,
    node_modules_only
});
...
if (result.success) {
    if (result.cleaned.length > 0) {
        ...
        console.log('✓ Nothing to clean (directories not found)');
    }
}
else {
    console.error(`❌ Cleaning failed for ${result.project_name}`);
    result.errors.forEach(error => console.error(`  ${error}`));
    process.exit(1);
}
