...
export namespace declarations {
    ...
    export type Package = p_.Refiner<
        ...
        s_in.Directory
    >
    export type Module = p_.Refiner_With_Parameter<
        s_out.Module,
        s_error.Error,
        s_in.Directory,
        {
...
export const Module: declarations.Module = ($, abort, $p) => {


    const commands_dir = p_.from.dictionary($).get_possible_entry(
        "commands",
    )
    const queries_dir = p_.from.dictionary($).get_possible_entry(
        "queries",
    )
    const temp_dir = p_.from.dictionary($).get_possible_entry(
        "temp",
    )
    return {
        'modules': p_.from.optional(p_.from.dictionary($).get_possible_entry(
            "modules",
        )).decide(
            ($): s_out.Module.modules => {
                if ($[0] !== 'directory') {
                    return abort(['not a directory', {
                        'internal path': $p.path,
                        'name': "modules",
                    }])
                }
                return p_.from.dictionary($[1]).map_and_aggregate_error<s_out.Module, s_error.Error>(
                    ($, id, abort): s_out.Module => {
                        const module_id = id
                        if ($[0] !== 'directory') {
                            return abort(['not a directory', {
                                'internal path': $p.path + "/modules",
                                'name': id,
                            }])
                        }
        ...
        'schemas': p_.from.optional(p_.from.dictionary($).get_possible_entry(
            "schemas",
        )).decide(
            ($): s_out.Schemas => {
                if ($[0] !== 'directory') {
...
export const Package: declarations.Package = ($, abort) => {
    ...
    return {
        'typescript': {
            'lib': {
                'src': Module(src_dir[1], abort, { 'path': "/typescript/lib/src" })
            }
        }
    }
}