        ...
        p_.decide.state($d.type, ($) => {
            switch ($[0]) {
                        ...
                        ($xx, id): pci.Command_Promise<d.All__Package_Error>[] => [
                            p_.decide.state($.instruction, ($) => {
                                ...
                                switch ($[0]) {
                                    ...
                                    case 'set up comparison': return p_.ss($, ($): pci.Command_Promise<d.All__Package_Error> => {
                ...
                case 'package': return p_.ss($, ($) => {
                    const path = $.path
                    return p_.decide.state($.instruction, ($) => {
                        switch ($[0]) {
                            case 'assert clean': return p_.ss($, ($) => $c['git assert is clean'].execute(
                                {
                            ...
                            case 'build and test': return p_.ss($, ($) => $c['build and test'].execute(
                                {
                                    'path': path,
                                },
                ...
                case 'project': return p_.ss($, ($) => {
                    ...
                    return p_.decide.state($.instruction, ($) => {
                        switch ($[0]) {
                            case 'analyze file structure': return p_.ss($, ($) => $c['analyze file structure'].execute(
                                {
                                    'path to project': path
                                },
                                ($): d.Error => ['get project files', $],
                            ))
                            case 'dependency graph': return p_.ss($, ($) => $c['create dependency graph'].execute(
                                {
                                    'path to project': path
                                },
                                ($): d.Error => ['dependency graph', $],
                            ))
                            case 'list file structure problems': return p_.ss($, ($) => $c['list file structure problems'].execute(
                                {
                                    'path to project': path
                                },
                                ($): d.Error => ['get project files', $],
                ...
                case 'publish': return p_.ss($, ($) => $c['publish'].execute(
                    $,
                    ($): d.Error => ['package', ['publish', $]],
                ))
                case 'set up comparison': return p_.ss($, ($) => p_change_context(
                    {
