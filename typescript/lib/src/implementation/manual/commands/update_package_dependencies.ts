...
export const $$: signatures.commands.update_package_dependencies = _pc.command_procedure(
    ($p, $cr, $q) => [
        ...
        $cr['npm update package dependencies'].execute(
            {
                'path': t_path_to_path.extend_context_path_with_list($p.path, { 'addition': _pc.list.literal(["typescript", "lib"]) }),
            },
        ...
        $cr['npm update package dependencies'].execute(
            {
                'path': t_path_to_path.extend_context_path_with_list($p.path, { 'addition': _pc.list.literal(["typescript", "test"])}),
            },
            ($) => ['error updating test', $],
        ),
        ...
        // _pc.if_.query(
        //     $q.stat(
        //         t_path_to_path.extend_context_path_with_list($p.path, { 'addition': _pc.list.literal(["typescript", "app"])}),
