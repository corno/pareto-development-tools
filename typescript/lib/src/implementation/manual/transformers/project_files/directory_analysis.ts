...
export namespace defined {
    ...
    export const Directory: interface_.defined.Directory = ($, $p) => {
        ...
        const dir = $
        return p_.decide.state($p['expected structure'], ($): d_out.Directory => {
            switch ($[0]) {
                ...
                case 'group': return p_.ss($, ($) => {
                    ...
                    return ['dictionary', dir.__d_map(($, id) => {
                        ...
                        ): d_out.Node => p_.decide.state($, ($): d_out.Node => {
                            switch ($[0]) {
                                case 'file': return p_.ss($, ($): d_out.Node => ['file', ({
                                    'structure': {
                                        'path': $p['structure path'],
                                        'classification': p_.decide.state($p['expected structure'], ($): d_out.Classification => {
                                            switch ($[0]) {
                                                case 'file': return p_.ss($, ($) => p_.decide.state($, ($) => {
                                                    switch ($[0]) {
                                                ...
                                                case 'directory': return p_.ss($, ($) => ['directory', p_.decide.state($, ($): d_out.Directory_Classification => {
                                                    switch ($[0]) {
                                    ...
                                    'unexpected path tail': p_.decide.state($p['expected structure'], ($) => {
                                        switch ($[0]) {
                                            case 'file': return p_.ss($, ($) => p_.decide.state($, ($) => {
                                                switch ($[0]) {
                                ...
                                case 'directory': return p_.ss($, ($): d_out.Node => {
                                    ...
                                    const dir = $
                                    return ['directory', p_.decide.state($p['expected structure'], ($): d_out.Directory => {
                                        switch ($[0]) {
                ...
                case 'dictionary': return p_.ss($, ($) => {
                    ...
                    return ['dictionary', dir.__d_map(($, id): d_out.Node => p_.decide.state($, ($): d_out.Node => {
                        switch ($[0]) {
    ...
    export const Directory: interface_.undefined.Directory = ($, $p) => {
        return ['dictionary', $.__d_map(($, id) => Node(
            $,
            {
                ...
                'unexpected path tail': p_.optional.from.optional($p['unexpected path tail']).map(($) => $ + "/" + id),
            }
        ))]
    }
            ...
            return p_.decide.state($, ($): d_out.Node => {
                switch ($[0]) {
                    ...
                    case 'file': return p_.ss($, ($): d_out.Node => ['file', {
                        ...
                        'extension': extension(id),
                        'unexpected path tail': p_.optional.block(() => {
                            ...
                            p_.optional.from.optional(possible_file_extension).map(($) => {
